...
import { getPostBySlug, getPageBySlug, getAllPosts, getAllPostsIncludingPrivates, getAllPages, getAllSettings, getAllPostSlugs } from '@lib/ghost'
...
export const getStaticPaths: GetStaticPaths = async () => {
  ...
  const postsAll = await getAllPostsIncludingPrivates(limitForPosts)
  const pages = await getAllPages(limitForPages)
  const settings = await getAllSettings()
  const { url: cmsUrl } = settings

  const postRoutes = (posts && postsAll as GhostPostsOrPages).map((post) => {
