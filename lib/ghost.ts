...
// ########### Ghost Web3 functionalities here ###########
...
const excludePostOrPageBySlug = () => {
  ...
  return 'tag:-full-private'
}

// Web3 tags filter
const includePostOrPageBySlug = () => {
  if (!contactPage) return ''
  return `tag:public, tag:private, tag:full-private`
}
...
export async function getAllPostsIncludingPrivates(props?: { limit: number }): Promise<GhostPostsOrPages> {
  const postsAll = await api.posts.browse({
    ...postAndPageFetchOptions,
    filter: includePostOrPageBySlug(),
    ...(props && { ...props }),
  })
  const results = await createNextProfileImagesFromPosts(postsAll)
  return await createNextFeatureImages(results)
}
